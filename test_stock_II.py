...
class TestStock(unittest.TestCase):
    ...
    def test_gbce_all_share_index(self):
        ...
        # Record individual trades for each stock - This does not have to be in order
        trades = [
    ...
    def test_performance_volume_weighted_stock_price(self):
        now = datetime.now()
        # Record trades for performance testing
        for i in range(1000):
            self.pop.record_trade(now - timedelta(seconds=i), 100, "buy", 105 + i % 10)

        # Measure performance of old_calculate_volume_weighted_stock_price
        start_time = time.time()
        old_vwsp = self.pop.old_calculate_volume_weighted_stock_price()
        old_duration = time.time() - start_time

        # Measure performance of calculate_volume_weighted_stock_price
        start_time = time.time()
